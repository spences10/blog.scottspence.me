...
I have been a Gatsby user since around [v0 May 2017], at that time I
was using a template that I cloned and used happily for this blog. It
was called Lumen and was just what I needed at the time.
...
It means that although there are a great deal of Gatsby starters and
themes out there to get you up and running in minutes, this post is
going to focus on what you need to do to build your own blog. Starting
with the most basic "Hello World!" to adding deploying your code to
production.

## What we're going to build

We're going to build a developer blog with MDX support for some React
components in Markdown goodness, so we will be able to add React
components into our Markdown posts.
...
`Dump` component, so we have added to the blog post we made:

````text
```jsx react-live
````

Now we can edit the code directly, try changing a few things like
this:
