...
On the homepage (`src/pages/index.js`) we're going to add the
following:
...
This will create the file path for the blog posts.
...
So, for each one of our blog posts we're going to use a template,
let's create a `templates` directory and a file file for that with:
...
For now we're going to scaffold out basic template, we're going to add
