...
need to add some additional information to the `siteMetadata` that
will be used by the SEO component, currently there's only a title and
description.
...
export default ({ data }) => {
  ...
  return (
    <Layout>
      <SEO title={title} description={description || 'nothin’'} />
      <IndexWrapper>
        {data.allMdx.nodes.map(
        ...
```
...
The task now is to add this information into the SEO component,
afterwards you can take a look at structured data (JSON-LD).

You already have `title` and `description` so time to add the
`canonical` link.

The canonical link is going to be pointing to the source of your
content. Example: if you share a post from your blog on DEV.to and
freeCodeCamp you can add a canonical link to them to identify your
blog as the original source and better improving your SEO ranking.

Add a `siteUrl` key to your `siteMetadata` on the `gatsby-config.js`
file.

Right now the validity of the URL isn't important as changes your
making are being validated locally. You will need to add the URL of
the site once it's deployed.
...
Add `siteUrl` to the `useSiteMetadata` hook and then add that to the
SEO component. As it's a fixed value the URL doesn't need to change
for the index page but it will for the individual blog posts, you can
get the full path of the URL from the `fields.slug` you added earlier.
...
Then pass the relevant props to the component from
`src/pages/index.js` and `src/templates/blogPostTemplate.js`.

Now is a good time to create an `seo` object to assign default values
to. Create this because you're either passing the variables via props
or getting them via the `useSiteMetadata` hook.

Destructure the `title`, `description` and `siteUrl` from the
`useSiteMetadata` hook and give them aliases, so the `title` will be
`title: defaultTitle`, same for `description` as
`description: defaultDescription`, `siteUrl` is ok as is. Now is also
a good time to add `siteLanguage` and `siteLocale`, they need to be
added to the `siteMetadata` object in `gatsby-config.js` as well as
...
You'll need to pass the `fields.slug` to the SEO component `pathname`
prop on `blogPostTemplate.js`:
...
Add a default image for the site, this will be used when sharing the
homepage (`/`) of the site. Save the image to the `static` folder and
call it `default-image.png`.

In your gatsby config create an `imageLink` key and point to the file
you just added to the `static` folder. Gatsby doesn't need the full
path of the file so, `/default-image.png` will be fine.

The `imageLink` will also need adding to the `useSiteMetadata` hook,
then to the SEO component props.

To get the image path from the blog post template you'll need to get
the `publicURL` from the `frontmatter.cover` property, your
`blogPostTemplate.js` query should look like this now:
...
To determine if the blog post has a cover image or not check the
`image` prop, if it's `'undefined'` then use the default image:

```js
const imagePath = `${siteUrl}${image ? 'undefined' : defaultImage}`;
```

Add in the following code for the Facebook and Twitter modules, first
Facebook:
...
If you haven't got a twitter username to add to the `siteMetadata`
then add the `twitterUsername` as `null` to the `seo` object and pass
that to the `Twitter` component as `username={seo.twitterUsername}`
update the `useSiteMetadata` hook to incorporate the new information
you have created also.

Last of all we want to add JSON-LD to the SEO component, this will
take `datePublished`, `dateModified` and `author`
...
Now to conditionally render the `schemaArticle` or the
`schemaOrgWebPage` depending if it's an article or not:
...
That was a lot you covered there! The completed components should look
like this:
  ...
  return (
    <Layout>
      <SEO
        ...
        pathname={fields.slug}
        article={true}
        image={cover.publicURL || defaultImage}
      />
      <h1>{frontmatter.title}</h1>
      ...
```
