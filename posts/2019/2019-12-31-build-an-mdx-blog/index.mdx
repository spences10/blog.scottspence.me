...
[Chris Biscardi] there's a tone of useful information in there on MDX
in Gatsby.
...
Cool, cool! Now we want to replace the code example with the props of
the child component of the pre component, we can do that with
...
Then to match the language, for now we're going to add in the
`matches` function to match the language class assigned to the code
block.
