...
[Stack Overflow], basically you need to ununstall Lxd with: 

```bash
sudo dpkg --force depends -P lxd; sudo dpkg --force depends -P lxd-client
```

Now we can go about updating Ubuntu wit the `do-release-upgrade` command, if you do that right away you may see something along the lines of:
